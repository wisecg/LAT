...
def plotSpecBeforeAfter():
    ...
    for ds in dsList:
        ...
        plt.savefig("./plots/lat3-comp-DS%s.pdf" % ds)
...
def plotSpectraAfter():
    ...
    # dsList = [0]
    plotName = "./plots/lat3-spec-DS-14-5BC.pdf"
    ...
    for ds in dsList: # indent this block
        ...
        if ds=="5B": bLo, bHi = 80, 112
        if ds=="5C": bLo, bHi = 113, 121
        fList = ["%s/bkg/cut/frb/frb_ds%d_%d_*.root" % (dsi.dataDir, dsNum, bIdx) for bIdx in range(bLo, bHi+1)]
