...
def checkROOTEff():
    ...
    for idx, ds in enumerate(dsList):
        ...
        hNewEnrHi = newF.Get('hDS{}_Enr_Sideband'.format(ds))
        hNewEnrLo = newF.Get('hDS{}_Enr_IS'.format(ds))

        hNewNat = newF.Get('hDS{}_Nat'.format(ds))
        hNewNatLo90 = newF.Get('hDS{}_Nat_Lo90'.format(ds))
        hNewNatHi90 = newF.Get('hDS{}_Nat_Hi90'.format(ds))
        ...
        hNewEnrHi90.SetLineStyle(ROOT.kDashed)
        hNewEnrLo90.SetLineStyle(ROOT.kDashed)
        hNewEnrHi90.SetLineColor(ROOT.kRed)
        hNewEnrLo90.SetLineColor(ROOT.kRed)
        hNewEnrHi.SetLineColor(ROOT.kBlue)
        hNewEnrLo.SetLineColor(ROOT.kMagenta)
        hNewEnr.Draw()
        hNewEnrHi.Draw("SAME")
        hNewEnrLo.Draw("SAME")
