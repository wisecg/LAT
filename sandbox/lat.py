...
Usage:
./lat.py [-i [plotNum] interactive mode]
         [-b batch mode -- creates new file]
         [-r [dsNum] [subNum] use skim range & sub-range]
...
def main(argv):
    ...
    intMode, batMode, rangeMode, fileMode, gatMode = False, False, False, False, False
    ...
    # File I/O
    inDir, outDir = "~/project/v2-waveskim", "~/project/lat"
    inFile, outFile, bltFile = TFile(), TFile(), TFile()
    gatTree, bltTree, oTree = TTree(), TTree(), TTree()
    theCut, inPath, outPath = "", "", ""
    if rangeMode:
        inPath = "%s/waveSkimDS%d_%d.root" % (inDir, dsNum, subNum)
        outPath = "%s/latSkimDS%d_%d.root" % (outDir, dsNum, subNum)
        inFile = TFile(inPath)
        gatTree = inFile.Get("skimTree")
        theCut = inFile.Get("theCut").GetTitle()
    if fileMode:
        inFull = "%s/waveSkimDS%d_run%d.root" % (inDir, dsNum, runNum)
        outPath = "%s/latSkimDS%d_run%d.root" % (outDir, dsNum, runNum)
        inFile = TFile(inFull)
        gatTree = inFile.Get("skimTree")
        theCut = inFile.Get("theCut").GetTitle()
    if gatMode:
        ...
        outPath = "%s/lat_run%d.root" % (outDir, runNum)
    ...
    if batMode:
        ...
        # print "Attempting tree copy to",outPath # TODO: Comment this back in!
    ...
    b4, b5, b6 = oTree.Branch("waveS3",waveS3), oTree.Branch("waveS3",waveS3), oTree.Branch("waveS3",waveS3)
    ...
    p = []
    for i in range(1,6): p.append(plt.subplot())
    if plotNum==0:
        p[0] = plt.subplot(111)
    ...
    while True:
        ...
        for iH in hitList:
            ...
            if intMode: print "%d:  run %d  chan %d  trapENFCal %.2f" % (iList, run, chan, dataENF)
    ...
    if batMode:
        ...
        print "and wrote",b_waveS0.GetEntries(),"entries in the new branches."
