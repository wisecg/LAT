...
def main(argv):
    ...
    for i, opt in enumerate(argv):
        ...
        # after running -scan, use this with or without fitSlo DB cut values to generate a h5 output
        if opt=="-pd":
            useDBCut = bool(argv[i+1])
            useWide = bool(argv[i+2])
            saveSloPanda(pctTot, useDBCut, useWide)
...
def scanRunsSlo(ds, key, mod, cIdx, pctTot):
    ...
    for iF, f in enumerate(fileList):
        ...
        for iE in range(tt.GetEntries()):
            ...
            # Widen energy window for including background for side band analysis
            # extending lower edge should be enough, can't really extend upper edge due to 2nd peak
            # if not 235.0 < sumET < 239.46: continue
...
def loadSloData(key, pctTot, bReturnPD=False, bWide=False):
    ...
    for ci in range(cal.GetIdxs(key)):
        if bWide:
            eFile = "%s/eff%d_%s_c%d_wide.npz" % (dsi.effDir, pctTot, key, ci)
        else:
        ...
        for i in range(len(evtHitE)):
            ...
            if bReturnPD:
                # The indexing here is for ordering to make sure
                #the lowest energy hit is always trapeNFCal1
                idxLo, idxHi = 0, 1
                if evtHitE[i][0] < evtHitE[i][1]:
                    idxLo, idxHi = 0, 1
                else:
                    idxLo, idxHi = 1, 0
                ...
                dfDict.setdefault('fitSlo2',[]).append(evtSlo[i][idxHi])
                dfDict.setdefault('riseNoise1',[]).append(evtRise[i][idxLo])
                dfDict.setdefault('riseNoise2',[]).append(evtRise[i][idxHi])
                dfDict.setdefault('ToE1',[]).append(evtToE[i][idxLo])
                dfDict.setdefault('ToE2',[]).append(evtToE[i][idxHi])

        eff["spec"].append(fSloSpec)
        eff["specX"] = fSloX # this doesn't change

        # If the dictionary exist, write into a dataframe (sometimes the number of events is 0!)
        if dfDict and bReturnPD:
            df = pd.DataFrame.from_dict(dfDict)
            # Change ordering of columns so it's pretty
            dfcols = ['key', 'cIdx', 'run', 'cpd1', 'cpd2','trapENFCal1', 'trapENFCal2', 'fitSlo1', 'fitSlo2', 'riseNoise1', 'riseNoise2', 'ToE1', 'ToE2']
            df = df[dfcols]
            dfList.append(df)
...
def saveSloPanda(pctTot, bUseDB=True, bWide=False):
    """
        ...
        You want to make sure that setSloCut is run with the DB values written before running with
        bUseDB = True
    """

    import pandas as pd

    # First save all the eff files into their own dataframe
    dfList = []
    for ds in [0,1,2,3,4,5,6]:
        for key in cal.GetKeys(ds):
    ...
    if bUseDB:
        ...
        # Dummy dictionary to store new data
        dFull = {}

        # Loop through all combinations, yes this is equivalent to 4 for loops
        for name, g in dfg:
            dbKey = "fitSlo_%s_idx%d_m2s238_eff%d" % (name[0], name[1], pctTot)
            cut1, cut2 = 0., 0.
            # Grab cuts from DB
            ds = [int(a) for a in name[0].split('_')[0] if a.isdigit()][0]

            fsD = dsi.getDBRecord(dbKey, calDB=calDB, pars=pars)
            ch1 = det.getCPDChan(ds, str(name[2]))
            ch2 = det.getCPDChan(ds, str(name[3]))

            # Check if BOTH detectors are active in the DB and have valid cuts -- otherwise skip
            if fsD and ch1 in fsD and ch2 in fsD:
                if fsD[ch1]:
                    cut1 = fsD[ch1][0]
                else:
                    print('key: {}_{} Ch1 {} in DB is empty! Skipping'.format(name[0], name[1], ch1))
                    continue

                if fsD[ch2]:
                    cut2 = fsD[ch2][0]
                else:
                    print('key: {}_{} Ch2 {} in DB is empty! Skipping'.format(name[0], name[1], ch2))
                    continue

                # Apply cut here and save values
                pass1 = g.apply(lambda x: x['fitSlo1'] < cut1, axis=1)
                pass2 = g.apply(lambda x: x['fitSlo2'] < cut2, axis=1)

                # Save again into a new dataframe
                # here I'm saving as a new dictionary, there may be a more efficient method to combine the DFs
                dFull.setdefault('key',[]).extend( len(g['trapENFCal1'].values.tolist())*[name[0]])
                dFull.setdefault('cIdx',[]).extend( len(g['trapENFCal1'].values.tolist())*[name[1]])
                dFull.setdefault('cpd1',[]).extend( len(g['trapENFCal1'].values.tolist())*[name[2]])
                dFull.setdefault('trapENFCal1',[]).extend(g['trapENFCal1'].values.tolist())
                dFull.setdefault('cpd2',[]).extend( len(g['trapENFCal2'].values.tolist())*[name[3]])
                dFull.setdefault('trapENFCal2',[]).extend(g['trapENFCal2'].values.tolist())
                dFull.setdefault('Pass1', []).extend(pass1.values.tolist())
                dFull.setdefault('Pass2', []).extend(pass2.values.tolist())
                dFull.setdefault('fitSlo1',[]).extend(g['fitSlo1'].values.tolist())
                dFull.setdefault('fitSlo2',[]).extend(g['fitSlo2'].values.tolist())
                dFull.setdefault('riseNoise1',[]).extend(g['riseNoise1'].values.tolist())
                dFull.setdefault('riseNoise2',[]).extend(g['riseNoise2'].values.tolist())
                dFull.setdefault('ToE1',[]).extend(g['ToE1'].values.tolist())
                dFull.setdefault('ToE2',[]).extend(g['ToE2'].values.tolist())
            else:
                continue

        dfFull = pd.DataFrame.from_dict(dFull)
        print('Saved full dataframe with cuts')
        print(dfFull.head())
        dfcols = ['key', 'cIdx', 'cpd1', 'cpd2','trapENFCal1', 'trapENFCal2', 'fitSlo1', 'fitSlo2', 'Pass1', 'Pass2', 'riseNoise1', 'riseNoise2', 'ToE1', 'ToE2']
        dfFull = dfFull[dfcols]
        dfFull.to_hdf('{}/data/CalPairHit_WithDbCut.h5'.format(os.environ['LATDIR']), 'skimTree', mode='w', format='fixed', complevel=9)

    # You probably don't want to save it like this, but this option is here for quick looks
    # in the future before setting up cuts in the DB
    else:
        df.to_hdf('{}/data/CalPairHitData_NoDbCut.h5'.format(os.environ['LATDIR']), 'skimTree', mode='w', format='fixed', complevel=9)
